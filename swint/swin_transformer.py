...
class SwishGLU(nn.Module):
    ...
    .. math::
        \text{SwishGLU}(x) = \text{Swish}(x) * \text{GLU}(x)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size, global_window_size, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        coords_local_flatten = torch.flatten(coords_local, 1)
        relative_coords_local = coords_local_flatten[:, :, None] - coords_local_flatten[:, None, :]
        relative_coords_local = relative_coords_local.permute(1, 2, 0).contiguous()
        relative_coords_local[:, :, 0] += self.window_size[0] - 1
        relative_coords_local[:, :, 1] += self.window_size[1] - 1
        relative_coords_local[:, :, 0] *= 2 * self.window_size[1] - 1
        relative_position_index_local = relative_coords_local.sum(-1)

        coords_h_global = torch.arange(self.global_window_size[0])
        coords_w_global = torch.arange(self.global_window_size[1])
        coords_global = torch.stack(torch.meshgrid([coords_h_global, coords_w_global]))
        coords_global_flatten = torch.flatten(coords_global, 1)
        relative_coords_global = coords_global_flatten[:, :, None] - coords_global_flatten[:, None, :]
        relative_coords_global = relative_coords_global.permute(1, 2, 0).contiguous()
        relative_coords_global[:, :, 0] += self.global_window_size[0] - 1
        relative_coords_global[:, :, 1] += self.global_window_size[1] - 1
        relative_coords_global[:, :, 0] *= 2 * self.global_window_size[1] - 1
        relative_position_index_global = relative_coords_global.sum(-1)

        self.register_buffer("relative_position_index_local", relative_position_index_local)
        self.register_buffer("relative_position_index_global", relative_position_index_global)
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias_local = self.relative_position_bias_table[:self.window_size[0] * self.window_size[1] ** 2, :].view(
        ...
        relative_position_bias_global = self.relative_position_bias_table[self.window_size[0] * self.window_size[1] ** 2:, :].view(
